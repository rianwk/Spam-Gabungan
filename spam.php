...
if(is_numeric($argv[1])) {
	echo "[]Prossesing Telp Tokopedia [] #".$G.$argv[1].$X."\n";
	...
	while(1) {
		echo "[]Spam PHD Sending Massage To[] # ".$G.$argv[1].$X."\n";	
...
if(!file_exists($argv[1])) {
	die($RR."[!] Penggunaan Salah [!]".$X."\n");
}
$argv[1]=explode("\n",file_get_contents($argv[1]));
$argv[1]=str_replace(' ','',$argv[1]);
foreach($argv[1] as $argv[2]):
echo "[]Prossessing Telp Tokopedia[] #".$G.$argv[2].$X."\n";
...
while(1) {
foreach($argv[1] as $argv[2]):
echo "[]Spam PHD Sending Massage To[] # ".$G.$argv[2].$X."\n";
